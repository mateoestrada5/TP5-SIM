...
def runge_kutta(t0=0, x0=0, h=0.001, xf=120, ec_a=0.5, ec_b=-0.2, ec_c=5):
    ...
    while x < xf:
        ...
        lineas_rk.append({"t": t, "x": x, "k1": k1, "k2": k2, "k3": k3, "k4": k4, "x_sig": x_sig})
        t += h
        x = x_sig



    lineas_rk.append({"t": t, "x": x, "k1": '', "k2": '', "k3": '', "k4": '', "x_sig": ''})

    t_final = round(t,4)
    x_final = round(x,4)

    runge_kutta_json["lineas"] = lineas_rk[:5] + lineas_rk[-5:]  # Keep only the first and last 5 lines for brevity
